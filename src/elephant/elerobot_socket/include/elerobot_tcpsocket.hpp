...
class TCPSocket : public BaseSocket
{
  public:
    ...
    int Send(const char *bytes, size_t byteslength) {
      if (this->isClosed) {
        return -1;
      } // `socket` has been closed. Send() will fail.
    ...
    void Connect(uint32_t ipv4, uint16_t port, std::function<void()> onConnected = [](){}, FDR_ON_ERROR)
    {
      ...
      // Called when the client control is in the process of establishing a connection with a RD Session Host server.
      onConnected(); // Connected to the server, fire the event.
    ...
    void Listen()
    {
      std::thread t(TCPSocket::Receive, this); // `this` is the parameter of `TCPSocket::Receive`
      t.detach(); 
    }
